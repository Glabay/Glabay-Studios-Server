package io.xeros.model;

/**
 * @author Zei | Glabay-Studios
 * @project Glabay-Studios-Server
 * @social Discord: Z
 * @social Github: <a href="https://github.com/Zeighe">Zeighe</a>
 * @since 10/5/2024
 */
public class Shops {

    public static final Integer GENERAL_SHOP = 2;
    public static final Integer GENERAL_SHOP_2 = 4;
    public static final Integer RANGE_SHOP = 4;
    public static final Integer MAGE_SHOP = 6;
    public static final Integer SKILLING_SHOP = 7;
    public static final Integer MELEE_SHOP = 8;
    public static final Integer DONATOR_SHOP = 9;
    public static final Integer SLAYER_SHOP = 10;
    public static final Integer MASTER_FARMER_SHOP = 16;
    public static final Integer GRACE_SHOP = 18;
    public static final Integer AVA_SHOP = 19;
    public static final Integer CRAFTING_SHOP = 20;
    public static final Integer FISHING_SHOP = 22;
    public static final Integer HUNTER_SHOP = 23;
    public static final Integer SIGMUND_THE_MERCHANT_SHOP = 26;
    public static final Integer IRONMAN_SHOP = 41;
    public static final Integer ZAFFS_MAGIC_SHOP = 47;
    public static final Integer RANGE_SUPPLIES_SHOP = 48;
    public static final Integer PEST_CONTROL_SHOP = 75;
    public static final Integer VOTE_SHOP = 77;
    public static final Integer EMBLEM_TRADER_SHOP = 80;
    public static final Integer OZIACH_SHOP = 81;
    public static final Integer NURMOF_SHOP = 90;
    public static final Integer BOB_BRILLIANT_AXES_SHOP = 91;
    public static final Integer CONSUMABLES_SHOP = 113;
    public static final Integer FANCY_SHOP = 114;
    public static final Integer DOOMSAYER_SHOP = 117;
}
