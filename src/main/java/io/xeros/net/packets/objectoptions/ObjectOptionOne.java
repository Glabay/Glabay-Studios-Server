...
public class ObjectOptionOne extends ObjectAction {
    ...
    public static void handleOption(final Player c, int objectType, int obX, int obY) {
        ...
        if (OutlastLeaderboard.handleInteraction(c, objectType, 1)) return;

        if (c.getGnomeAgility().gnomeCourse(c, objectType)) return;
        if (c.getWildernessAgility().wildernessCourse(c, objectType)) return;
        if (c.getBarbarianAgility().barbarianCourse(c, objectType)) return;
        if (c.getBarbarianAgility().barbarianCourse(c, objectType)) return;
        if (c.getAgilityShortcuts().agilityShortcuts(c, objectType)) return;

        if (RooftopSeers.execute(c, objectType)) return;
        if (c.getRooftopFalador().execute(c, objectType)) return;
        if (RooftopVarrock.execute(c, objectType)) return;
        if (RooftopArdougne.execute(c, objectType)) return;
        if (c.getRoofTopDraynor().execute(c, objectType)) return;
        if (c.getRooftopAlkharid().execute(c, objectType)) return;


        if (c.getRooftopPollnivneach().execute(c, objectType)) return;
        if (c.getRooftopRellekka().execute(c, objectType)) return;
        if (c.getLighthouse().execute(c, objectType)) return;
        ...
        if (IntStream.of(HUNTER_OBJECTS).anyMatch(id -> objectType == id)) {
            if (Hunter.pickup(c, object)) return;
            if (Hunter.claim(c, object)) return;
        }
        ...
        if (door != null && DoorHandler.clickDoor(c, door)) return;
        ...
        switch (objectType) {
                ...
                PlayerHandler.nonNullStream().filter(p -> Boundary.isIn(p, Boundary.HESPORI))
                    .forEach(players -> {
                        ...
                        if (p != null && !players.getLoginName().equalsIgnoreCase(c.getLoginName()))
                            if (p.getMacAddress().equalsIgnoreCase(c.getMacAddress())) c.canEnterHespori = false;
                    });
            ...
            case 33311:
                ...
                break;
            case 1521:
                if (c.absX == 2958) c.getPA().movePlayer(2957, 3820, 0);
                else if (c.absX == 2957) c.getPA().movePlayer(2958, 3820, 0);
                else if (c.absX == 2908) c.getPA().movePlayer(2908, 9697, 0);
                break;
            case 36197:
            ...
            case 34727:
                ...
                break;
            case 21600:
                if (c.absY == 3802) c.getPA().movePlayer(2326, 3801, 0);
                else if (c.absY == 3801) c.getPA().movePlayer(2326, 3802, 0);
                break;
            case 31990:
                if (c.absY == 4054) Vorkath.exit(c);
                else if (c.absY == 4052) Vorkath.enterInstance(c, 10);
                break;
            case 34548:
            ...
            case 6150:
                if (c.getItems().playerHasItem(barType[0])) c.getSmithingInt().showSmithInterface(barType[0]);
                else if (c.getItems().playerHasItem(barType[1])) c.getSmithingInt().showSmithInterface(barType[1]);
                else if (c.getItems().playerHasItem(barType[2])) c.getSmithingInt().showSmithInterface(barType[2]);
                else if (c.getItems().playerHasItem(barType[3])) c.getSmithingInt().showSmithInterface(barType[3]);
                else if (c.getItems().playerHasItem(barType[4])) c.getSmithingInt().showSmithInterface(barType[4]);
                else if (c.getItems().playerHasItem(barType[5])) c.getSmithingInt().showSmithInterface(barType[5]);
                else c.sendMessage("You don't have any bars.");
                break;
            case 11846:
                if (c.combatLevel >= 100) if (c.getY() > 5175) Highpkarena.addPlayer(c);
                else Highpkarena.removePlayer(c, false);
                else if (c.combatLevel >= 80) if (c.getY() > 5175) Lowpkarena.addPlayer(c);
                else Lowpkarena.removePlayer(c, false);
                else c.sendMessage("You must be at least level 80 to compete in events.");
                break;
            case 2996:
            ...
            case 34662:
                ...
                else c.getDH().sendStatement("@red@You need a Konar slayer key to open this.");
                break;
            case 34544: //Karuulm Rocks/Stone Bars (Intro)
                if (c.absX == 1320 && c.absY == 10205 || c.absX == 1320 && c.absY == 10206) AgilityHandler.delayEmote(c, "JUMP", 1322, c.absY, 0, 2);
                else if (c.absX == 1322 && c.absY == 10205 || c.absX == 1322 && c.absY == 10206)
                    AgilityHandler.delayEmote(c, "JUMP", 1320, c.absY, 0, 2);
                if (c.absX == 1311 && c.absY == 10214 || c.absX == 1312 && c.absY == 10214) AgilityHandler.delayEmote(c, "JUMP", 1311, 10216, 0, 2);
                else if (c.absX == 1311 && c.absY == 10216 || c.absX == 1312 && c.absY == 10216)
                    AgilityHandler.delayEmote(c, "JUMP", 1312, 10214, 0, 2);
                if (c.absX == 1303 && c.absY == 10206 || c.absX == 1303 && c.absY == 10205) AgilityHandler.delayEmote(c, "JUMP", 1301, 10205, 0, 2);
            ...
            case 11845:
                if (c.combatLevel >= 100) {
                    if (c.getY() < 5169) Highpkarena.removePlayer(c, false);
                }
                else if (c.combatLevel >= 80) {
                    if (c.getY() < 5169) Lowpkarena.removePlayer(c, false);
                }
                else c.sendMessage("You must be at least level 80 to compete in events.");
            ...
            case 10068:
                if (c.getZulrahEvent().isStarting()) c.sendMessage("Your Zulrah instance is about to start.");
                ...
                else c.getZulrahEvent().initialize();
                break;
            case 12941:
            ...
            case 31624:
                ...
                for (int skill = 0; skill < c.playerLevel.length; skill++) {
                    ...
                    if (c.playerLevel[skill] < c.getLevelForXP(c.playerXP[skill])) {
                        ...
                        if (c.playerLevel[skill] > c.getLevelForXP(c.playerXP[skill])) c.playerLevel[skill] = c.getLevelForXP(c.playerXP[skill]);
...
//					} else {
                ...
                break;
            case 7811:
                if (!c.getPosition().inClanWarsSafe()) return;
            ...
            case 1206:
                ...
                if (!c.getMode().isOsrs() && !c.getMode().is5x()) c.getPA().addSkillXP(200, 19, true);
                else c.getPA().addSkillXP(10, 19, true);
                break;
            ...
            case 14918:
                ...
                if (c.absY > 3808) AgilityHandler.delayEmote(c, "JUMP", 3201, 3807, 0, 2);
                else AgilityHandler.delayEmote(c, "JUMP", 3201, 3810, 0, 2);
                break;

            case 29728:
                if (c.absY > 3508) AgilityHandler.delayEmote(c, "JUMP", 1722, 3507, 0, 2);
                else AgilityHandler.delayEmote(c, "JUMP", 1722, 3512, 0, 2);
                break;
            ...
            case 28893:
                ...
                if (c.absY > 10064) AgilityHandler.delayEmote(c, "JUMP", 1610, 10062, 0, 2);
                else AgilityHandler.delayEmote(c, "JUMP", 1613, 10069, 0, 2);
                break;

            case 27987: // scorpia
                if (c.absX == 1774) AgilityHandler.delayEmote(c, "CLIMB_DOWN", 1769, 3849, 0, 2);
                else AgilityHandler.delayEmote(c, "CLIMB_UP", 1774, 3849, 0, 2);
                break;
            ...
            case 27985:
                if (c.absY > 3872) AgilityHandler.delayEmote(c, "CLIMB_DOWN", 1761, 3871, 0, 2);
                else AgilityHandler.delayEmote(c, "CLIMB_UP", 1761, 3874, 0, 2);
                break;
            ...
            case 29730:
                if (c.absX > 1604) AgilityHandler.delayEmote(c, "JUMP", 1603, 3571, 0, 2);
                else AgilityHandler.delayEmote(c, "JUMP", 1607, 3571, 0, 2);
                break;

            case 25014:
                if (Boundary.isIn(c, Boundary.PURO_PURO)) c.getPA().startTeleport(2525, 2916, 0, "puropuro", false);
                else c.getPA().startTeleport(2592, 4321, 0, "puropuro", false);
                break;
            ...
            case 4311:// Mining Guild Entrance
                if (c.absY == 3697) c.getPA().movePlayer(2681, 3698, 0);
                else if (c.absY == 3698) c.getPA().movePlayer(2682, 3697, 0);
                break;
            case 30366:// Mining Guild Entrance
                if (c.absX == 3043 && c.absY == 9730) if (c.playerLevel[Player.playerMining] >= 60) c.getPA().movePlayer(3043, 9729, 0);
                else c.sendMessage("You must have a mining level of 60 to enter.");
                else if (c.absX == 3043 && c.absY == 9729) c.getPA().movePlayer(3043, 9730, 0);
                break;

            case 30365:// Mining Guild Entrance
                if (c.absX == 3019 && c.absY == 9733) if (c.playerLevel[Player.playerMining] >= 60) c.getPA().movePlayer(3019, 9732, 0);
                else c.sendMessage("You must have a mining level of 60 to enter.");
                else if (c.absX == 3019 && c.absY == 9732) c.getPA().movePlayer(3019, 9733, 0);
                break;
            ...
            case 11470:
                ...
                else if (c.absY == 3368) c.getPA().movePlayer(3106, 3369, 0);
                else if (c.absY == 3364) c.getPA().movePlayer(3103, 3363, 0);
                break;
            case 21505:
            case 21507:
                if (c.absX == 2328) c.getPA().movePlayer(2329, 3804, 0);
                else if (c.absX == 2329) c.getPA().movePlayer(2328, 3804, 0);
                break;
            case 36556:
                ...
                else c.start(new CrystalCaveEntryDialogue(c));
                break;
            case 36691:
                c.objectDistance = 3;
            ...
            case 1569:// edge dung
                //edge dung
                if (c.absX == 3145) c.getPA().walkTo(+1, 0);
                else //edge dung
                    if (c.absX == 3146) c.getPA().walkTo(-1, 0);
                //edge dung
                if (c.absX == 3103) c.getPA().walkTo(+1, 0);
                else //edge dung
                    if (c.absX == 3104) c.getPA().walkTo(-1, 0);
                //wildy dung
                if (c.absX == 3022) c.getPA().walkTo(+1, 0);
                else //wildy dung
                    if (c.absX == 3023) c.getPA().walkTo(-1, 0);
                //wildy dung
                if (c.absX == 3040) c.getPA().walkTo(+1, 0);
                else //wildy dung
                    if (c.absX == 3041) c.getPA().walkTo(-1, 0);
                //wildy dung
                if (c.absX == 3044) c.getPA().walkTo(+1, 0);
                else //wildy dung
                    if (c.absX == 3045) c.getPA().walkTo(-1, 0);
                //edge dung
                if (c.absX == 3103) c.getPA().walkTo(+1, 0);
                else //edge dung
                    if (c.absX == 3104) c.getPA().walkTo(-1, 0);
                //edge dung
                if (c.absY == 9944 || c.absY == 9943) c.getPA().movePlayer(3106, 9945, 0);
                else //edge dung
                    if (c.absY == 9945 || c.absY == 9946) c.getPA().movePlayer(3106, 9944, 0);
                break;
            case 14910:
                if (c.absY == 3288) c.getPA().walkTo(0, +1);
                else if (c.absY == 3289) c.getPA().walkTo(0, -1);
                break;
            case 1727:
            case 1728:
                if (c.absY == 3903) c.getPA().walkTo(0, +1);
                else if (c.absY == 3904) c.getPA().walkTo(0, -1);
                if (c.absY == 3895) c.getPA().walkTo(0, +1);
                else if (c.absY == 3896) c.getPA().walkTo(0, -1);
                if (c.absY == 9917) c.getPA().walkTo(0, +1);
                else if (c.absY == 9918) c.getPA().walkTo(0, -1);
                if (c.absY == 3856) c.getPA().walkTo(0, -1);
                else if (c.absY == 3855) c.getPA().walkTo(0, +1);
                if (c.absY == 3182) c.getPA().walkTo(+1, 0);
                else if (c.absY == 3183) c.getPA().walkTo(-1, 0);
                if (c.absX == 3008) c.getPA().walkTo(-1, 0);
                else if (c.absX == 3007) c.getPA().walkTo(+1, 0);
                break;
            ...
            case 10042:
                ...
                break;
            case 17385:
                if (Boundary.isIn(c, Boundary.EDGE_DUNG_LADDER)) c.sendMessage("This area is currently closed.");
                else if (Boundary.isIn(c, Boundary.EDGE_DUNG_ENTRANCE_LADDER)) AgilityHandler.delayEmote(c, "CLIMB_UP", 3084, 3501, 0, 2);
                else if (Boundary.isIn(c, Boundary.FOE_DUNGEON)) AgilityHandler.delayEmote(c, "CLIMB_UP", 3077, 3501, 0, 2);
                break;
            case 33318:
            ...
            case 25938:
            case 11794:
                if (Boundary.isIn(c, Boundary.EDGEVILLE_EXTENDED)) AgilityHandler.delayEmote(c, "CLIMB_UP", c.absX, c.absY, 1, 2);
                break;
            case 25939:
            case 11795:
                if (Boundary.isIn(c, Boundary.EDGEVILLE_EXTENDED)) AgilityHandler.delayEmote(c, "CLIMB_DOWN", c.absX, c.absY, 0, 2);
                break;
            ...
            case 27785:
                ...
                break;
            case 30266:
                if (c.usedFc) c.getPA().movePlayer(2495, 5174, 0);
            ...
            case 4031:
                if (c.absY == 3117) {
                    if (EquipmentSet.DESERT_ROBES.isWearing(c)) c.getDiaryManager().getDesertDiary().progress(DesertDiaryEntry.PASS_GATE_ROBES);
                    else c.getDiaryManager().getDesertDiary().progress(DesertDiaryEntry.PASS_GATE);
                    c.getPA().movePlayer(c.absX, 3115);
                }
                else c.getPA().movePlayer(c.absX, 3117);
                break;
            ...
            case 11780:
                if (c.getDiaryManager().getVarrockDiary().hasCompleted("HARD")) if (c.absX == 3255)
            ...
            case 23104:
                ...
                else c.sendMessage("Please wait a few seconds between clicks.");
                break;

            case 21772:
                if (!Boundary.isIn(c, Boundary.CERBERUS_ROOM_WEST)) return;
            ...
            case 23105:
                ...
                if (c.absY == 1241) c.getPA().walkTo(0, +2);
                else c.moveTo(Cerberus.EXIT);
                break;
             ...
             */
            case 10230:
                if (c.getMode().isIronmanType()) c.getPA().movePlayer(2899, 4449, 4);
                else c.getPA().movePlayer(2899, 4449, 0);
                break;
            ...
            case 26724:
                ...
                if (c.getX() == 2427 && c.getY() == 9767) c.getPA().movePlayer(2427, 9762);
                else if (c.getX() == 2427 && c.getY() == 9762) c.getPA().movePlayer(2427, 9767);
                break;
            case 535:
                if (obX == 3722 && obY == 5798) if (c.getMode().isIronmanType()) c.getPA().movePlayer(3677, 5775, 4);
                else c.getPA().movePlayer(3677, 5775, 0);
                break;

            case 536:
                if (obX == 3678 && obY == 5775) c.getPA().movePlayer(3723, 5798);
                break;

            case 26720:
                if (obX == 2427 && obY == 9747) {
                    if (c.getX() == 2427 && c.getY() == 9748) c.getPA().movePlayer(2427, 9746);
                    else if (c.getX() == 2427 && c.getY() == 9746) c.getPA().movePlayer(2427, 9748);
                }
                else if (obX == 2420 && obY == 9750) {
                    if (c.getX() == 2420 && c.getY() == 9751) c.getPA().movePlayer(2420, 9749);
                    else if (c.getX() == 2420 && c.getY() == 9749) c.getPA().movePlayer(2420, 9751);
                }
                else if (obX == 2418 && obY == 9742) {
                    if (c.getX() == 2418 && c.getY() == 9741) c.getPA().movePlayer(2418, 9743);
                    else if (c.getX() == 2418 && c.getY() == 9743) c.getPA().movePlayer(2418, 9741);
                }
                else if (obX == 2357 && obY == 9778) {
                    if (c.getX() == 2358 && c.getY() == 9778) c.getPA().movePlayer(2356, 9778);
                    else if (c.getX() == 2356 && c.getY() == 9778) c.getPA().movePlayer(2358, 9778);
                }
                else if (obX == 2388 && obY == 9740) {
                    if (c.getX() == 2389 && c.getY() == 9740) c.getPA().movePlayer(2387, 9740);
                    else if (c.getX() == 2387 && c.getY() == 9740) c.getPA().movePlayer(2389, 9740);
                }
                else if (obX == 2379 && obY == 9738) if (c.getX() == 2380 && c.getY() == 9738) c.getPA().movePlayer(2378, 9738);
                else if (c.getX() == 2378 && c.getY() == 9738) c.getPA().movePlayer(2380, 9738);
                break;

            case 26721:
                if (obX == 2358 && obY == 9759) if (c.getX() == 2358 && c.getY() == 9758) c.getPA().movePlayer(2358, 9760);
                else if (c.getX() == 2358 && c.getY() == 9760) c.getPA().movePlayer(2358, 9758);
                if (obX == 2380 && obY == 9750) if (c.getX() == 2381 && c.getY() == 9750) c.getPA().movePlayer(2379, 9750);
                else if (c.getX() == 2379 && c.getY() == 9750) c.getPA().movePlayer(2381, 9750);
                break;
            ...
            case 534:
                if (obX == 3748 && obY == 5760) c.getPA().movePlayer(2356, 9782, 0);
                break;
            case 9706:
                if (obX == 3104 && obY == 3956) c.getPA().startLeverTeleport(3105, 3951, 0);
                break;

            case 9707:
                if (obX == 3105 && obY == 3952) c.getPA().startLeverTeleport(3105, 3956, 0);
                break;
            case 3610:
                if (obX == 3550 && obY == 9695) c.getPA().startTeleport(3565, 3308, 0, "modern", false);
                break;
            case 26561:
                if (obX == 2913 && obY == 5300) c.getPA().movePlayer(2914, 5300, 1);
                break;
            case 26562:
                if (obX == 2920 && obY == 5274) c.getPA().movePlayer(2920, 5274, 0);
                break;
            case 26504:
                if (c.absX == 2909 && c.absY == 5265) c.getGodwars().enterBossRoom(God.SARADOMIN);
                if (c.absX == 2907 && c.absY == 5265) c.getPA().movePlayer(2909, 5265);
                break;
            case 26518:
                if (obX == 2885 && obY == 5333) c.getPA().movePlayer(2885, 5344);
                else if (obX == 2885 && obY == 5344) c.getPA().movePlayer(2885, 5333);
                break;
            case 26505:
                if (c.absX == 2925 && c.absY == 5333) c.getGodwars().enterBossRoom(God.ZAMORAK);
                if (c.absX == 2925 && c.absY == 5331) c.getPA().movePlayer(2925, 5333);
                break;
            case 26503:
                if (c.absX == 2862 && c.absY == 5354) c.getGodwars().enterBossRoom(God.BANDOS);
                if (c.absX == 2864 && c.absY == 5354) c.getPA().movePlayer(2862, 5354);
                break;
            case 26380:
                if (obX == 2871 && obY == 5270) if (c.getY() == 5279) c.getPA().movePlayer(2872, 5269);
                else if (c.getY() == 5269) c.getPA().movePlayer(2872, 5279);
                break;
            ...
            case 10:
                ...
                if (c.heightLevel == 0) AgilityHandler.delayEmote(c, "CLIMB_UP", 3372, 9645, 1, 2);
                else AgilityHandler.delayEmote(c, "CLIMB_DOWN", 3372, 9645, 0, 2);
                break;
            case 26502:
                if (c.absX == 2839 && c.absY == 5294) c.getGodwars().enterBossRoom(God.ARMADYL);
                if (c.absX == 2839 && c.absY == 5296) c.getPA().movePlayer(2839, 5294);
                break;
            case 172:
            case 170:
                c.objectDistance = 3;
            ...
            case 11803:
                ...
                break;
            case 17387:
                if (c.getRights().isOrInherits(Right.ONYX_CLUB)) AgilityHandler.delayEmote(c, "CLIMB_UP", 2125, 4913, 0, 2);
                break;
            case 25824:
                c.facePosition(obX, obY);
            ...
            case 2320:
            case 23566:
                if (c.absY == 9964 || c.absY == 9963) c.getPA().movePlayer(3120, 9970, 0);
                else if (c.absY == 9969 || c.absY == 9970) c.getPA().movePlayer(3120, 9963, 0);
                break;
            case 26760:
                if (c.absX == 3184 && c.absY == 3945) c.getDH().sendDialogues(631, -1);
                else if (c.absX == 3184 && c.absY == 3944) c.getPA().movePlayer(3184, 3945, 0);
                break;
            case 19206:
            ...
            case 9326:
                ...
                if (c.absX < 2769) c.getPA().movePlayer(2775, 10003, 0);
                else c.getPA().movePlayer(2768, 10002, 0);
                break;
            case 4496:
            case 4494:
                if (c.heightLevel == 2) c.getPA().movePlayer(3412, 3540, 1);
                else if (c.heightLevel == 1) c.getPA().movePlayer(3418, 3540, 0);
                break;
            case 9319:
                if (c.heightLevel == 0)
            ...
            case 9320:
                ...
                break;
            case 4493:
                if (c.heightLevel == 0) c.getPA().movePlayer(c.absX - 5, c.absY, 1);
                else if (c.heightLevel == 1) c.getPA().movePlayer(c.absX + 5, c.absY, 2);
                break;

            case 4495:
                if (c.heightLevel == 1 && c.absY > 3538 && c.absY < 3543) c.getPA().movePlayer(c.absX + 5, c.absY, 2);
                else c.sendMessage("I can't reach that!");
                break;
            case 2623:
                if (c.absX == 2924 && c.absY == 9803) c.getPA().movePlayer(c.absX - 1, c.absY, 0);
                else if (c.absX == 2923 && c.absY == 9803) c.getPA().movePlayer(c.absX + 1, c.absY, 0);
                break;
            case 15644:
            case 15641:
            case 24306:
            case 24309:
                ...
                if (c.heightLevel == 0) if (c.absX == 2855 || c.absX == 2854) {
                    if (c.absY == 3546)
            ...
            case 16671:
                ...
                if (distanceToPoint < 5) c.getPA().movePlayer(2840, 3539, 2);
                break;
            ...
            case 14888:
                if (!c.getItems().playerHasItem(Items.RING_MOULD) && !c.getItems().playerHasItem(Items.AMULET_MOULD)
                    ...
                    if (c.getItems().playerHasItem(Items.BRACELET_MOULD)) BraceletMaking.craftBraceletDialogue(c);
                }
                else JewelryMaking.mouldInterface(c);
                break;
            ...
            case 28851:// wcgate
                ...
                else c.getPA().movePlayer(1657, 3505, 0);
                break;
            case 28852:// wcgate
                if (c.playerLevel[8] < 60) {
                    c.sendMessage("You need a Woodcutting level of 60 to enter the Woodcutting Guild.");
                ...
                else c.getPA().movePlayer(1657, 3504, 0);
                break;
            case 2309:
                if (c.getX() == 2998 && c.getY() == 3916) c.getAgility().doWildernessEntrance(c, 2998, 3916, false);
                if (c.absX == 2998 && c.absY == 3917) c.getPA().movePlayer(2998, 3916, 0);
                break;
            case 1766:
                if (c.getPosition().inWild() && c.absX == 3069 && c.absY == 10255) c.getPA().movePlayer(3017, 3850, 0);
                break;
            case 1765:
                if (c.getPosition().inWild() && c.absY >= 3847 && c.absY <= 3860) c.getPA().movePlayer(3069, 10255, 0);
                break;

            case 2118:
                if (Boundary.isIn(c, new Boundary(3433, 3536, 3438, 3539))) c.getPA().movePlayer(3438, 3537, 0);
                break;

            case 2114:
                if (Boundary.isIn(c, new Boundary(3433, 3536, 3438, 3539))) c.getPA().movePlayer(3433, 3537, 1);
                break;


            case 7108:
            case 7111:
                if (c.absX == 2907 || c.absX == 2908) if (c.absY == 9698) c.getPA().walkTo(0, -1);
                else if (c.absY == 9697) c.getPA().walkTo(0, +1);
                break;

            case 2119:
                if (c.heightLevel == 1) if (c.absX == 3412 && (c.absY == 3540 || c.absY == 3541)) c.getPA().movePlayer(3417, c.absY, 2);
                break;

            case 2120:
                if (c.heightLevel == 2) if (c.absX == 3417 && (c.absY == 3540 || c.absY == 3541)) c.getPA().movePlayer(3412, c.absY, 1);
                break;

            case 2102:
            case 2104:
                if (c.heightLevel == 1) if (c.absX == 3426 || c.absX == 3427) if (c.absY == 3556) c.getPA().walkTo(0, -1);
                else if (c.absY == 3555) c.getPA().walkTo(0, +1);
                break;
            ...
            case 1596:
                // case 7408:
                // case 7407:
                if (c.absY < 9000) if (c.absY > 3903) c.getPA().movePlayer(c.absX, c.absY - 1, 0);
                else c.getPA().movePlayer(c.absX, c.absY + 1, 0);
                else if (c.absY > 9917) c.getPA().movePlayer(c.absX, c.absY - 1, 0);
                else c.getPA().movePlayer(c.absX, c.absY + 1, 0);
                break;
            ...
            case 272:
                ...
                else c.getPA().movePlayer(c.absX, c.absY, 1);
                break;
            case 273:
                if (c.absY == 3956 || c.absY == 3957) c.getPA().movePlayer(3018, 3958, 0);
                else c.getPA().movePlayer(c.absX, c.absY, 0);
                break;
            /* Godwars Door */
            /*
            ...
            case 5960://lever at magebank
                ...
                break;
            case 5959:
                if (c.absX != 3089) c.getPA().startLeverTeleport(2539, 4712, 0);
                break;
            case 1814:
                ...
                break;
            case 1535:
                if (c.absX == 2564 && c.absY == 3310) c.getPA().movePlayer(2563, 3310, 0);
                else if (c.absX == 2563 && c.absY == 3310) c.getPA().movePlayer(2674, 9479, 0);
                break;
            case 14929:
            ...
            case 5103:
                if (c.absX == 2691 && c.absY == 9564) c.getPA().movePlayer(2689, 9564, 0);
                else if (c.absX == 2689 && c.absY == 9564) c.getPA().movePlayer(2691, 9564, 0);
                break;

            case 5106:
            case 21734:
                if (c.absX == 2674 && c.absY == 9479) c.getPA().movePlayer(2676, 9479, 0);
                else if (c.absX == 2676 && c.absY == 9479) c.getPA().movePlayer(2674, 9479, 0);
                break;
            case 5105:
            case 21733:
                if (c.absX == 2672 && c.absY == 9499) c.getPA().movePlayer(2674, 9499, 0);
                else if (c.absX == 2674 && c.absY == 9499) c.getPA().movePlayer(2672, 9499, 0);
                break;

            case 5107:
            case 21735:
                if (c.absX == 2693 && c.absY == 9482) c.getPA().movePlayer(2695, 9482, 0);
                else if (c.absX == 2695 && c.absY == 9482) c.getPA().movePlayer(2693, 9482, 0);
                break;

            case 21731:
                if (c.absX == 2691) c.getPA().movePlayer(2689, 9564, 0);
                else if (c.absX == 2689) c.getPA().movePlayer(2691, 9564, 0);
                break;

            case 5104:
            case 21732:
                if (c.absX == 2683 && c.absY == 9568) c.getPA().movePlayer(2683, 9570, 0);
                else if (c.absX == 2683 && c.absY == 9570) c.getPA().movePlayer(2683, 9568, 0);
                break;

            case 5100:
                if (c.absY <= 9567) c.getPA().movePlayer(2655, 9573, 0);
                else if (c.absY >= 9572) c.getPA().movePlayer(2655, 9566, 0);
                break;
            case 21728:
            ...
            case 21727:
                ...
                if (c.objectX == 2698 && c.objectY == 9498) c.getPA().movePlayer(2698, 9492, 0);
                else if (c.objectX == 2698 && c.objectY == 9493) c.getPA().movePlayer(2698, 9499, 0);
                break;
            case 5088:
            case 20882:
            ...
            case 16511:
                ...
                else if (c.absX == 3155 && c.absY == 9906) c.getPA().movePlayer(3149, 9906, 0);
                break;
            ...
            case 1551:
                if (c.absX == 3252 && c.absY == 3266) c.getPA().movePlayer(3253, 3266, 0);
                if (c.absX == 3253 && c.absY == 3266) c.getPA().movePlayer(3252, 3266, 0);
                break;
            case 1553:
                if (c.absX == 3252 && c.absY == 3267) c.getPA().movePlayer(3253, 3266, 0);
                if (c.absX == 3253 && c.absY == 3267) c.getPA().movePlayer(3252, 3266, 0);
                break;
            case 3044:
            case 24009:
            case 26300:
            ...
            case 2030:
                c.objectDistance = 1;
                if (CannonballSmelting.isSmeltingCannonballs(c)) CannonballSmelting.smelt(c);
                else c.getSmithing().sendSmelting();
                break;
            /*
            ...
            case 13287:
                ...
                else c.sendMessage("This bank is only for Ultimate Ironman to use.");
                break;
            case 3506:
            case 3507:
                if (c.absY == 3458)
             ...
             */
            case 6552:
                if (c.getPosition().inWild()) return;
                c.autocasting = false;
                c.autocastId = -1;
                c.getPA().resetAutocast();
                if (c.absY == 9312) {
            ...
            case 20377:
                ...
                else c.sendMessage("You already have full prayer points.");
                break;
            case 61:
                if (c.getPosition().inWild()) return;
                if (c.absY >= 3508 && c.absY <= 3513) if (c.playerLevel[5] < c.getPA().getLevelForXP(c.playerXP[5])) {
                    if (Boundary.isIn(c, Boundary.VARROCK_BOUNDARY)
                        && c.getDiaryManager().getVarrockDiary().hasCompleted("HARD"))
                        if (c.prayerActive[25]) c.getDiaryManager().getVarrockDiary().progress(VarrockDiaryEntry.PRAY_WITH_PIETY);
                    c.startAnimation(645);
                    c.playerLevel[5] = c.getPA().getLevelForXP(c.playerXP[5]);
                    c.sendMessage("You recharge your prayer points.");
                    c.getPA().refreshSkill(5);
                ...
                break;
            case 36201:
                if (Raids.isMissingRequirements(c)) return;
            ...
            case 410:
                ...
                if (Boundary.isIn(c, Boundary.TAVERLY_BOUNDARY)) if (c.getItems().isWearingItem(5574) && c.getItems().isWearingItem(5575)
                    && c.getItems().isWearingItem(5576)) c.getDiaryManager().getFaladorDiary().progress(FaladorDiaryEntry.ALTAR_OF_GUTHIX);
                c.startAnimation(645);
                c.playerLevel[5] = c.getPA().getLevelForXP(c.playerXP[5]);
                c.sendMessage("You recharge your prayer points.");
                c.getPA().sendSound(2674);
            ...
            case 29941:
                ...
                if (Boundary.isIn(c, Boundary.VARROCK_BOUNDARY))
                    if (c.prayerActive[23]) c.getDiaryManager().getVarrockDiary().progress(VarrockDiaryEntry.PRAY_WITH_SMITE);
                if (Boundary.isIn(c, Boundary.ARDOUGNE_BOUNDARY)) if (c.prayerActive[25]) {
            ...
            case 411:
                if (c.playerLevel[5] < c.getPA().getLevelForXP(c.playerXP[5])) {
                    if (c.getPosition().inWild()) c.getDiaryManager().getWildernessDiary().progress(WildernessDiaryEntry.WILDERNESS_ALTAR);
                    c.startAnimation(645);
                    c.playerLevel[5] = c.getPA().getLevelForXP(c.playerXP[5]);
                    c.sendMessage("You recharge your prayer points.");
                    c.getPA().sendSound(2674);
            ...
            case 11737:
                if (!c.getRights().isOrInherits(Right.ONYX_CLUB)) return;
            ...
            case 1557:
            case 7169:
                if ((c.objectX == 3106 || c.objectX == 3105) && c.objectY == 9944) if (c.getY() > c.objectY)
                    c.getPA().walkTo(0, -1);
                else
                    c.getPA().walkTo(0, 1);
                else if (c.getX() > c.objectX)
                    c.getPA().walkTo(-1, 0);
                else
                    c.getPA().walkTo(1, 0);
                break;
            ...
            case 9294:
                if (c.absX < c.objectX) c.getPA().movePlayer(c.objectX + 1, c.absY, 0);
                else if (c.absX > c.objectX) c.getPA().movePlayer(c.objectX - 1, c.absY, 0);
                break;

            case 9293:
                if (c.absX < c.objectX) c.getPA().movePlayer(2892, 9799, 0);
                else c.getPA().movePlayer(2886, 9799, 0);
                break;
            ...
            case 3203:
                ...
                if (!Boundary.isIn(c, Boundary.DUEL_ARENA)) return;
